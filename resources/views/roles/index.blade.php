                    ...
                    <tbody class="text-gray-700 dark:text-gray-200 text-sm font-light">
                        @foreach ($roles as $role)
                            <tr class="border-b border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600">
                                ...
                                <td class="py-3 px-6 text-left">{{ $role->permissions->pluck('name')->implode(', ') }}</td>
                                <td class="py-3 px-6 text-center flex items-center justify-center space-x-2">
                                    <!-- Botón de Editar -->
                                    <a href="{{ route('roles.edit', $role->id) }}" class="text-blue-500 hover:text-blue-700 transition duration-150 ease-in-out inline-flex items-center">
                                    ...
                                    <form action="{{ route('roles.destroy', $role->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-500 hover:text-red-700 transition duration-150 ease-in-out inline-flex items-center" onclick="return confirm('¿Estás seguro de que deseas eliminar este rol?')">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
